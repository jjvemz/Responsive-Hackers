...
const HackerNews = () => {
  ...
  const [favorites, setFavorites] = useState(
    "favorites" in window.localStorage
  ...
  const getNews = async () => {
    ...
    await axios
      ...
      .then((response) => {
        setNews(
          JSON.stringify(
            response.data.hits.filter(
              ({ author, story_title, story_url, created_at, ..._ }) =>
                author && story_title && created_at && story_url
            )
          )
        );
        return news;
    ...
    <div className="Front-End-Test---Home-view">
      ...
      <div>
        {JSON.parse(news)
          .filter(
            (_, index) =>
              (page - 1) * newsPerPage <= index && index < newsPerPage * page
          )
          .map((item) => {
            return (
              <Container>
                <Grid  item xs={12} md={6} lg={4} key={item.objectID}>
                    <div className="Rectangle">
                      ...
                      >
                        <span className="-hours-ago-by-autho">
                          ...
                          {now.diff(item.created_at, "weeks")} days ago by{" "}
                          {item.author}
                        </span>
                        <br />
                      ...
                      <div>
                        <button>
                          {!favorites.includes(item.objectID) ? (
                            <img
                              src="../../assets/iconmonstrfavorite3.svg"
                              alt="Open Heart"
                              onClick={() => {
                            ...
                            <img
                              src="../../assets/iconmonstrfavorite2.svg"
                              alt="Empty Heart"
                              onClick={() => {
                                console.log("desclickeado");
                                setFavorites(
                                  favorites.filter((id) => id !== item.objectID)
                                );
                                window.localStorage.setItem(
                                  "favorites",
                                  favorites
      ...
      {JSON.parse(news).length <= newsPerPage ? (
        <></>
      ) : (
        <Paginate
          setPage={setPage}
          numberOfPages={Math.ceil(JSON.parse(news).length / newsPerPage)}
        />
      )}
    </div>
  );
